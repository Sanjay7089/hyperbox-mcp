"""Phase 3: mount external MCP servers into sandbox-mcp as live proxies.
...
- codebase indexing -> prefix `index`. DESIGN.md offers mcp-code-indexer
  ...
  external service, which keeps `python -m sandbox_mcp.server` runnable
...
stopping sandbox-mcp from starting. Set either env var below to "off"
...
DOCS_URL = os.environ.get("SANDBOX_MCP_DOCS_URL", "https://mcp.context7.com/mcp")

# The indexer is a stdio server. Override to swap in mcp-code-indexer,
# e.g. SANDBOX_MCP_INDEX_CMD="mcp-code-indexer --qdrant-url http://...".
INDEX_CMD = os.environ.get("SANDBOX_MCP_INDEX_CMD", "npx -y semantic-search-mcp")
