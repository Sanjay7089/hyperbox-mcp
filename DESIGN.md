...
| 2026-09-04 | Phase 3 mounts semantic-search-mcp (not mcp-code-indexer) under `index`, Context7 under `docs` | Both were sanctioned options; the local one needs no Qdrant service, so `python -m sandbox_mcp.server` stays runnable with nothing else set up. Overridable via SANDBOX_MCP_INDEX_CMD / SANDBOX_MCP_DOCS_URL, either set to "off" to skip that mount |
